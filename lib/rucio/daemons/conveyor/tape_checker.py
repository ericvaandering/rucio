...
"""
Conveyor tape checker is a daemon to check the tape status
"""
...
def tape_checker(once=False, activities=None, sleep_time=60,
    ...
    time.sleep(10)  # To prevent running on the same partition if all the tape checkers restart at the same time
    heart_beat = heartbeat.live(executable, hostname, pid, hb_thread)
    prepend_str = 'Thread [%i/%i] : ' % (heart_beat['assign_thread'] + 1, heart_beat['nr_threads'])
        ...
        try:
            ...
            for activity in activities:
                ...
                transfs = request_core.get_next(request_type=[RequestType.TRANSFER, RequestType.STAGEIN, RequestType.STAGEOUT],
                                                state=[RequestState.TAPE_CHECKING],
                                                limit=db_bulk,
                ...
                record_timer('daemons.conveyor.tape-checker.000-get_next', (time.time() - start_time) * 1000)
                ...
                for external_host in xfers_ids:
                    ...
                    for xfers in chunks(external_ids, fts_bulk):
                        # poll transfers
                        check_tape_status(external_host=external_host, xfers=xfers, prepend_str=prepend_str, request_ids=request_ids, timeout=timeout)
...
def run(once=False, sleep_time=60, activities=None,
    ...
    if once:
        logging.info('executing one tape checker iteration only')
        tape_checker(once=once, fts_bulk=fts_bulk, db_bulk=db_bulk, older_than=older_than, activities=activities, activity_shares=activity_shares)

    else:

        logging.info('starting tape checker threads')

        threads = [threading.Thread(target=tape_checker,
...
def check_tape_status(external_host, xfers, prepend_str='', request_ids=None, timeout=None):
    """
