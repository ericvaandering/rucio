...
create_rules = function(approval) {
    ...
    options['error'] = function(jqXHR, textStatus, errorThrown) {
        ...
        } else if (error_details['ExceptionClass'] == 'RSEBlacklisted') {
            html_check = '<font color="red">The RSE that you chose is currently not available for writing. Please check <a href="http://atlas-agis.cern.ch/agis/ddmblacklisting/list/" target="_blank">here</a> for more information.</font>';>>>>>>> c78798acb... More language
        } else {
...
clear_form = function() {
    ...
    jira_alert = storage.get('jira_alert');
    storage.removeAll();
    storage.set('tour_alert', tour_alert);
    storage.set('jira_alert', jira_alert)
    $("#pattern_input").val("");
    $("#rse_input").val("");
...
$(document).ready(function() {
    ...
    $(document).on('close.fndtn.alert', function(event) {
        ...
        if (event.target.id == 'jira_alert') {
            storage.set('jira_alert', false);
        }
    });
    if (storage.get('tour_alert') == false) {
        $("#tour_alert").hide();
    }
    if (storage.get('jira_alert') == false) {
        $("#jira_alert").hide();
    }
